-- Enable realtime notifications using Broadcast
-- Broadcast is recommended for trigger-based inserts per Supabase docs

-- 1. RLS policy on realtime.messages for broadcast authorization
...
-- 2. Update trigger to broadcast after inserting notification
...
BEGIN
    ...
    IF OLD.status != 'completed' AND NEW.status = 'completed' THEN
        ...
        -- Broadcast for realtime updates
        PERFORM realtime.broadcast_changes(
            'notifications:' || NEW.organisation_id::text,
            'INSERT', 'INSERT', 'notifications', 'public',
            to_jsonb(notification_record), '{}'::jsonb
        );
    END IF;
