-- Simplify to public broadcast channels
-- The channel name 'notifications:{org_id}' provides sufficient isolation
-- Private channels require complex RLS setup that isn't working correctly

-- Drop the complex RLS policy
DROP POLICY IF EXISTS "Users can receive broadcasts for their organisations" ON "realtime"."messages";

-- Restore simple authenticated policy (public channels still require auth)
...
-- Update trigger to use public broadcast (FALSE = public)
...
BEGIN
    ...
    IF OLD.status != 'completed' AND NEW.status = 'completed' THEN
        ...
        -- Broadcast via PUBLIC channel (FALSE = public, simpler than private)
