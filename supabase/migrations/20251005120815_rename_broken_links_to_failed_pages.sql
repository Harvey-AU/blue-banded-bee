-- Change broken_links to count failed tasks instead of HTTP 4xx status codes
-- Remove total_404s metric entirely
-- Rename total_broken_links to total_failed_pages for clarity
    ...
    IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
        ...
        WITH task_stats AS (
            SELECT
                ...
                COUNT(*) FILTER (WHERE status = 'failed') AS total_failed_pages,
        ...
        SELECT jsonb_build_object(
            -- Basic counts
            'total_failed_pages', COALESCE(total_failed_pages, 0),
...
-- Update calculation version to v3.0 to track this change
COMMENT ON FUNCTION calculate_job_stats() IS 'Calculate comprehensive job statistics. v3.0: Changed total_broken_links to total_failed_pages counting WHERE status=failed instead of HTTP 4xx codes. Removed total_404s metric.';
