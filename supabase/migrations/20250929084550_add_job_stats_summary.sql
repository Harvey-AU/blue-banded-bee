    ...
    IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
        ...
        WITH task_stats AS (
            SELECT
                ...
                -- Detailed slow page buckets
                COUNT(*) FILTER (WHERE response_time > 10000) AS pages_over_10s,
                COUNT(*) FILTER (WHERE response_time >= 5000 AND response_time < 10000) AS pages_5_to_10s,
                COUNT(*) FILTER (WHERE response_time >= 3000 AND response_time < 5000) AS pages_3_to_5s,
                COUNT(*) FILTER (WHERE response_time >= 2000 AND response_time < 3000) AS pages_2_to_3s,
                COUNT(*) FILTER (WHERE response_time >= 1500 AND response_time < 2000) AS pages_1_5_to_2s,
                COUNT(*) FILTER (WHERE response_time >= 1000 AND response_time < 1500) AS pages_1_to_1_5s,
                COUNT(*) FILTER (WHERE response_time >= 500 AND response_time < 1000) AS pages_500ms_to_1s,
                COUNT(*) FILTER (WHERE response_time < 500) AS pages_under_500ms,
                ...
                -- Response time statistics with full percentile distribution
                AVG(response_time) FILTER (WHERE response_time IS NOT NULL) AS avg_response_time_ms,
                MIN(response_time) FILTER (WHERE response_time IS NOT NULL) AS min_response_time_ms,
                MAX(response_time) FILTER (WHERE response_time IS NOT NULL) AS max_response_time_ms,
                PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS p25_response_time_ms,
                PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS median_response_time_ms,
                PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS p75_response_time_ms,
                PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS p90_response_time_ms,
                PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS p95_response_time_ms,
                PERCENTILE_CONT(0.99) WITHIN GROUP (ORDER BY response_time) FILTER (WHERE response_time IS NOT NULL) AS p99_response_time_ms,
        ...
        response_time_buckets AS (
            ...
            SELECT
                jsonb_build_object(
                    'under_100ms', COUNT(*) FILTER (WHERE response_time < 100),
                    '100_500ms', COUNT(*) FILTER (WHERE response_time >= 100 AND response_time < 500),
                    '500_1000ms', COUNT(*) FILTER (WHERE response_time >= 500 AND response_time < 1000),
                    '1_3s', COUNT(*) FILTER (WHERE response_time >= 1000 AND response_time < 3000),
                    'over_3s', COUNT(*) FILTER (WHERE response_time >= 3000)
                ) as buckets
            FROM tasks
            WHERE job_id = NEW.id AND response_time IS NOT NULL
        ),
        ...
        SELECT jsonb_build_object(
            ...
            -- Performance metrics with full percentile breakdown
...
-- Create trigger to calculate stats when job completes
CREATE TRIGGER trigger_calculate_job_stats
    BEFORE UPDATE OF status ON jobs
    FOR EACH ROW
    EXECUTE FUNCTION calculate_job_stats();

-- Add comment for documentation
COMMENT ON COLUMN jobs.stats IS 'Comprehensive job statistics calculated upon completion, including performance metrics, error counts, and cache effectiveness';
