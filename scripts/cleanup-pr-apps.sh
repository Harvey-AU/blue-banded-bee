...
FLY_APPS=$(flyctl apps list | grep "blue-banded-bee-pr-" | awk '{print $1}' || true)
...
for APP in $FLY_APPS; do
    # Extract PR number from app name (e.g., blue-banded-bee-pr-114 -> 114)
    PR_NUM=$(echo "$APP" | sed 's/blue-banded-bee-pr-//')
